...
> **Definition 1.** Suppose an integer sequence $\lbrace a_i \rbrace_{i \in I}$ satisfies $a_i \neq 0$ for $i > 0$. We write the simple continued fraction of $\lbrace a_i \rbrace_i$ as follows:
>
> $$x = [a_0; a_1; \cdots] = a_0 + \cfrac{1}{a_1 + \cfrac{1}{a_2 + \cfrac{1}{\ddots}}}$$

Then by the Euclidean algorithm, we can show the following:

> **Proposition 1.** Every $q \in \mathbb{Q}_{\geq 0}$ can be represented as a simple continued fraction of some finite sequence $\lbrace a_i \rbrace_{i \in I}$.[^1] Moreover, we can arrange it so that $a_0 \geq 0$ and $a_i > 0$ for $i > 0$.
